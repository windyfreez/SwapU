      ...
      <div v-if="orders.length > 0" class="orders-list">
        ...
        >
          <div class="order-header">
            ...
            <span class="order-status" :class="getStatusClass(order.status)">{{ getStatusText(order) }}</span>
          </div>
          ...
          <div class="order-body">
            ...
            <div class="order-side">
              ...
              <div class="order-actions">
                ...
                <template v-if="currentRole === 'seller' && order.status === 7">
                  <button class="btn btn-primary btn-sm" @click.stop="handleAudit(order, 'refund', 'approve')">同意退货</button>
                  <button class="btn btn-outline btn-sm" @click.stop="handleAudit(order, 'refund', 'reject')">拒绝退货</button>
                </template>
                <template v-if="currentRole === 'seller' && order.status === 9">
                  <button class="btn btn-primary btn-sm" @click.stop="handleAudit(order, 'cancel', 'approve')">同意取消</button>
                  <button class="btn btn-outline btn-sm" @click.stop="handleAudit(order, 'cancel', 'reject')">拒绝取消</button>
                </template>

                <!-- 买家操作 -->
                <button v-if="currentRole === 'buyer' && order.status === 1" class="btn btn-outline btn-sm" @click.stop="handleCancel(order)">取消订单</button>
                <button v-if="currentRole === 'buyer' && order.status === 2" class="btn btn-primary btn-sm" @click.stop="handlePay(order)">去支付</button>
                <button v-if="currentRole === 'buyer' && order.status === 2" class="btn btn-outline btn-sm" @click.stop="handleCancel(order)">申请取消</button>
                <button v-if="currentRole === 'buyer' && order.status === 3" class="btn btn-outline btn-sm" @click.stop="handleRefundApply(order)">申请退货退款</button>
                <button v-if="currentRole === 'buyer' && order.status === 4" class="btn btn-primary btn-sm" @click.stop="handleConfirm(order)">确认收货</button>
                <span v-if="currentRole === 'buyer' && isWaitingAudit(order.status)" class="order-hint">等待卖家审核</span>
    ...
    <div v-if="showCancelModal" class="modal-overlay" @click="closeModal">
      <div class="modal-content" @click.stop>
        ...
        <div class="modal-body">
          <p v-if="selectedOrder && selectedOrder.status === 2" class="audit-tip">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="confirmCancel">
            {{ selectedOrder && selectedOrder.status === 2 ? '提交申请' : '确认取消' }}
          </button>
        </div>
      </div>
    </div>
...
const cancelTitle = computed(() =>
  selectedOrder.value && selectedOrder.value.status === 2 ? '申请取消订单' : '取消订单'
)
...
const confirmCancel = async () => {
  const result = await applyCancel(selectedOrder.value.orderNo, cancelReason.value)
  if (!result.ok) {
    alert(result.msg)
    return
  }
...
const confirmRefundApply = async () => {
  const result = await applyRefund(selectedOrder.value.orderNo, refundReason.value)
  if (!result.ok) {
    alert(result.msg)
    return
  }
...
const handleAudit = (order, type, action) => {
  selectedOrder.value = order
