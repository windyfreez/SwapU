...
为提高系统性能，使用 **Redis** 作为缓存中间件，实现热门商品以及库存信息缓存。使用 **RabbitMQ** 作为消息队列，解耦订单核心流程与非核心业务。使用 **JWT** 实现用户身份鉴权，采用拦截器校验用户Token。系统聊天模块采用 **WebSocket** 技术实现实时通信，支持买卖双方即时消息交互。同时通过**阿里云OSS**存储提供文件上传服务用于商品图片存储。整体架构具有良好的扩展性、可维护性和高并发处理能力。
